...
uniform vec4 iMouse;
#define iiMouse iMouse.xy / iResolution.xy
#define iiTime iTime
...
float getwaves(vec2 position, int iterations){
    ...
    for(int i=0;i<iterations;i++){
        ...
        vec2 res = wavedx(position, p, speed, phase, iiTime);
...
vec3 getRay(vec2 uv){
    uv = (uv * 2.0 - 1.0) * vec2(iiResolution.x / iiResolution.y, 1.0);
    vec3 proj = normalize(vec3(uv.x, uv.y, 1.0) + vec3(uv.x, uv.y, -1.0) * pow(length(uv), 2.0) * 0.05);
    if(iiResolution.x < 400.0) return proj;
    vec3 ray = rotmat(vec3(0.0, -1.0, 0.0), 3.0 * ((iiMouse.x + 0.5) * 2.0 - 1.0)) * rotmat(vec3(1.0, 0.0, 0.0), 1.5 * ((iiMouse.y + 0.5) * 2.0 - 1.0)) * proj;
    return ray;
}
...
{
    vec2 uv = fragCoord.xy / iiResolution.xy;
